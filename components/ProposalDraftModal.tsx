      ...
      <html lang="en">
        ...
        <body>
          ...
          <div class="section">
            ...
            <div class="section-content">${editedContent[s.id] ?? s.content}</div>
          </div>`).join('')}
  ...
  const handleWhatsAppShare = () => {
    ...
    const message = isUrdu
      ? `*ٹینڈر تعمیل خلاصہ*\n*Karez 2.0 — PPRA کمپلائنس انجن*\n\n` +
        ...
        `*جمع کروانے کی آخری تاریخ:* ${tenderData?.basicInfo?.submissionDeadline || 'ٹینڈر دستاویز دیکھیں'}\n` +
        ...
        `_نوٹ: [INSERT] والے حصے جمع کروانے سے پہلے آپ کو خود بھرنے ہوں گے۔_\n` +
      ...
      : `*TENDER COMPLIANCE SUMMARY*\n*Karez 2.0 — PPRA Compliance Engine*\n\n` +
        ...
        `*Submission Deadline:* ${tenderData?.basicInfo?.submissionDeadline || 'Check tender document'}\n` +
        ...
        `_Note: Sections marked [INSERT] require your input before submission._\n` +
                  ...
                  {proposalData.sections && proposalData.sections.length > 0 ? (
                    proposalData.sections.map((section: any, idx: number) => (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-800/80 pb-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() =>
                                handleCopyText(
                                  `${section.sectionNumber || ''} ${section.title}\n\n${editedContent[section.id || `sec-${idx}`] ?? section.content}`,
                                  section.id || `sec-${idx}`
                                )
                              }
                        ...
                        {/* Section Content */}
                        {editingSection === (section.id || `sec-${idx}`) ? (
                          <div className="mt-3">
                            <textarea
                              className="w-full bg-gray-800 border border-emerald-500 rounded-lg p-3 text-sm text-white resize-none focus:outline-none focus:ring-1 focus:ring-emerald-400"
                              rows={8}
                              value={editedContent[section.id || `sec-${idx}`] ?? section.content}
                              onChange={(e) => setEditedContent(prev => ({ 
                                ...prev, 
                                [section.id || `sec-${idx}`]: e.target.value 
                              }))}
                            />
                              ...
                              <button
                                onClick={() => {
                                  setEditedContent(prev => {
                                    const next = { ...prev };
                                    delete next[section.id || `sec-${idx}`];
                                    return next;
                                  });
                                  setEditingSection(null);
                                }}
